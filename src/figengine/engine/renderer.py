...
class RenderEngine:
    ...
    def draw_layout(layout_data: Dict[str, Any], bg_color: Union[str, Tuple[int, int, int]] = "white") -> PILImage.Image:
        ...
        for i, item in enumerate(placements):
            ...
            # Row 中使用的是垂直对齐 (v_align: top, center, bottom, full)
            ...
            # 需要先等比缩放至宽度一致，然后通过 pad_to_size 填充高度
                ...
                padded_img = temp_img.pad_to_size(
                    ...
                    loc=align_mode, # 映射 row.v_align -> image.loc
                    axis="height", 
                    bg_color=bg_color 
                )
...
class TextRenderer:
    ...
    def render(text: str, 
        ...
        # 2. 创建画布
        # 给定足够大的画布防止旋转被裁剪
        fig = Figure(figsize=(box_width_inch, box_height_inch), dpi=dpi)
        
        # 强制将 Matplotlib 画布背景设为全透明
