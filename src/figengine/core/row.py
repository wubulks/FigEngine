...
class Row:
    ...
    def __init__(self, 
                 ...
                 v_align: Literal["top", "center", "bottom", "full"] = "top",
        ...
        :param v_align: 行内垂直对齐方式 (type: str, default: "top")。
                        options:
                        - "full": 强制两端对齐 (Justified)。图片会自动缩放以等高并填满行宽。
                        - "top": 顶部对齐 (垂直方向)。
                        - "center": 垂直居中对齐。
                        - "bottom": 底部对齐。
        :param h_align: 行内容的水平对齐方式 (type: str, default: "full")。
                        ...
                        - "full": 按行宽等比缩放图片 (间距固定)。
                        - "justify": 自动分配行内额外空隙，但保留首尾的 left/right gap。
        """
        ...
        if self.v_align not in valid_aligns:
            raise ValueError(f"Invalid row vertical alignment '{self.v_align}'. Options: {valid_aligns}")

        self.h_align = h_align.lower()
        Validator.validate_horizontal_alignment(self.h_align)
