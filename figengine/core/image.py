...
class Image:
    ...
    def __init__(self, source: Union[str, PILImage.Image], label: Optional[str] = None, dpi: Optional[int] = None):
        ...
        # 加载逻辑委托给 Engine
        if isinstance(source, str):
            self._pil_image = IOEngine.load(source)
            file_dpi = self._pil_image.info.get('dpi')
            self.dpi = dpi if dpi else (int(file_dpi[0]) if file_dpi else 300)
    ...
    def crop(self, 
             ...
             unit: Literal["pixel", "ratio", "inch", "cm", "mm"] = "pixel") -> 'Image':
        """
        ...
        :param unit: 数值单位 (type: str, default: "pixel")。
    ...
    def add_ticks(self, 
        ...
        for x in x_points:
            ...
            current_img = current_img.add_text(
                ...
                rotation=0, anchor="top", **text_kwargs
            )
            ...
            current_img = current_img.add_text(
                ...
                rotation=0, anchor="bottom", **text_kwargs
            )
