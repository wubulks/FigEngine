...
Date: 2026/01/18
License: MIT License
Description: Figure orchestration class. Manages the layout lifecycle: 
...
class Figure:
    ...
    def add_row(self, items: List[Union[str, Image]], 
                ...
                unit: Literal["pixel", "ratio", "inch", "cm", "mm"] = "ratio",
                align: Literal["full", "top", "center", "bottom"] = "top",
                ) -> 'Figure':
        """
        向画布添加一行图片 (Row)。
        ...
        :param unit: 间距数值的单位 (type: str, default: "ratio")。
                     options: ["pixel", "ratio", "inch", "cm", "mm"]。
        :param align: 行内图片的对齐/排版方式 (type: str, default: "top")。
                      options:
        ...
        logger.debug(f"Added row {len(self.rows)} (Count: {len(items)}, Align: {align})")
        return self
    ...
    def replace_row(self, index: int, 
        ...
        """
        [New] Replace an existing row at a specific index.
        
        Useful for tweaking a specific part of the layout without recreating the whole Figure.
        
        :param index: The 0-based index of the row to replace.
        :param items: New list of images/paths.
        # ... (other parameters are the same as add_row) ...
        """
        if index < 0 or index >= len(self.rows):
            raise IndexError(f"Row index out of range: {index}. Current row count: {len(self.rows)}")

        # 1. Prepare Image objects
        img_objects = []
        for item in items:
            if isinstance(item, str):
                img_objects.append(Image(item))
        ...
        # 3. Replace the row in the list
        self.rows[index] = new_row
        
        # 4. Mark layout as dirty to force recalculation
        self._is_dirty = True
        self._cached_image = None
    ...
    def remove_row(self, index: int) -> 'Figure':
        """
        [New] Remove a row at a specific index.
        """
        if index < 0 or index >= len(self.rows):
    ...
    def get_row(self, index: int) -> Row:
        """
        [New] Get the Row object at a specific index for inspection.
        """
        if index < 0 or index >= len(self.rows):
            raise IndexError(f"Row index out of range: {index}")
