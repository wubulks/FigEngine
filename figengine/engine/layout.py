...
class LayoutEngine:
    ...
    def calculate(rows: List[Any], 
        ...
        for i, row in enumerate(rows):
            ...
            # 逻辑：保持原图相对大小关系不变，统一缩放以填满宽度。
            # 取缩放后最高的图作为“行高”，矮的图需要填充背景。
            else:
                # 1. 计算缩放比例 (Fit Width)
                sum_original_width = sum([img.width for img in row.images])
                
                scale_factor = 1.0
                if sum_original_width > 0:
                ...
                logger.debug(f"Row {i} ({row.align.upper()}): height={row_final_height}px, scale={scale_factor:.2f}")
            ...
            for j, img in enumerate(row.images):
                ...
                placements.append({
                    ...
                    "width": target_w,
                    "height": target_h,
                    "align": row.align  # 传递对齐方式给 RenderEngine
                })
