...
class Validator:
    ...
    def validate_numeric_param(value: Union[int, float], name: str, min_val: float = 0):
        ...
        """
        # logger.debug(f"Validating numeric param '{name}': {value}") # 过于频繁，可视情况注释掉
    ...
    def validate_position(position: str):
        ...
        # 注意: Image.py 中还支持 "left", "right", "top", "bottom" 等简写，
        # 这里的校验集合可能需要根据 Image._parse_position 的逻辑进行扩充，
        # 目前保持原逻辑。
        valid_positions = {"top_left", "top_right", "bottom_left", "bottom_right", "center", 
                           "top", "bottom", "left", "right"} # [扩充以匹配 Image.py]
    ...
    def validate_horizontal_alignment(alignment: str):
        ...
        :param alignment: 对齐字符串 (e.g. "left")。
        """
